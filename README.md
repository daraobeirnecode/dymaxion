...
- **Capabilities**: three implemented native capabilities — the Phase 0 `inspect_dataset` GeoJSON slice, the Phase 1A read-only `inspect_arcgis_org` organization inventory ([docs](docs/capabilities/inspect-arcgis-org.md)), and the Phase 1B read-only `trace_arcgis_dependencies` dependency graph ([docs](docs/capabilities/trace-arcgis-dependencies.md)) — plus 45 historical Sprint 1 skill scaffolds; folder presence is not a production-readiness claim, and the remaining roadmap capabilities are not implemented
